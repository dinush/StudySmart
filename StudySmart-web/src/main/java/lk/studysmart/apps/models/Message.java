...
@NamedQueries({
    ...
    @NamedQuery(name = "Message.findByFourOrs", query = "SELECT m FROM Message m WHERE (m.targetuser = :user OR m.userlevel = :userlevel OR m.class1 = :class2 OR m.grade = :grade) AND (m.type <> 5) ORDER BY m.addeddate DESC")})
